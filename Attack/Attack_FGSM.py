...
def FGSM_Attack(model, fmodel, example, label):
    # 每次只攻击一个样本
    sucess = 0
...
if __name__ == "__main__":
    ...
    for idx, x in tqdm(enumerate(X_train)):
        y = Y_train[idx]
        ad_image, ad_label, success = FGSM_Attack(model=model, fmodel=fmodel, example=x, label=y)
        if success == 1:
            sucess_cnt += 1
    ...
    # 保存
    np.savez(os.path.join(base_path, "../Adv/Mnist/Lenet5/FGSM_perclass={:d}.npz".format(int(ADV_NUM / 10))),
    ...
    # np.savez(os.path.join(base_path, "../Adv/Cifar10/Lenet5/FGSM_perclass={:d}.npz".format(int(ADV_NUM/10))),
    #          X_train_clean=X_train_clean, X_train_adv=X_train_adv, train_label=train_label,
    #          X_test_clean=X_test_clean, X_test_adv=X_test_adv, test_label=test_label)

    # np.savez(os.path.join(base_path, "../Adv/Cifar10/VGG11/FGSM_perclass={:d}.npz".format(int(ADV_NUM/10))),
    #          X_train_clean=X_train_clean, X_train_adv=X_train_adv, train_label=train_label,
    #          X_test_clean=X_test_clean, X_test_adv=X_test_adv, test_label=test_label)
