...
def PGD_Attack(model, fmodel, example, label):
    # 每次只攻击一个样本
    sucess = 0

    attack = fb.attacks.PGD(fmodel)
    ad_siganl = attack(example, label=label)  # 最大的迭代步长
    # ad_label = np.argmax(model.predict(np.array([ad_siganl])))
...
if __name__ == "__main__":
    ...
    # 加载模型
    model = load_model(os.path.join(base_path, "../Model_weight/Mnist/Lenet5/Sat-Jun-18-21:52:57-2022.h5"))
    ...
    # 加载数据集
    X_train, Y_train, _, _, X_test, Y_test = load_mnist(onehot=False, sequence=False, split=False)
    ...
    for idx, x in tqdm(enumerate(X_train)):
        y = Y_train[idx]
        ad_image, ad_label, success = PGD_Attack(model=model, fmodel=fmodel, example=x, label=y)
        if success == 1:
            sucess_cnt += 1
    ...
    # 保存
    np.savez(os.path.join(base_path, "../Adv/Mnist/Lenet5/PGD_perclass={:d}.npz".format(int(ADV_NUM / 10))),
    ...
    # np.savez(os.path.join(base_path, "../Adv/Cifar10/Lenet5/PGD_perclass={:d}.npz".format(int(ADV_NUM / 10))),
    #          X_train_clean=X_train_clean, X_train_adv=X_train_adv, train_label=train_label,
    #          X_test_clean=X_test_clean, X_test_adv=X_test_adv, test_label=test_label)

    # np.savez(os.path.join(base_path, "../Adv/Cifar10/VGG11/PGD_perclass={:d}.npz".format(int(ADV_NUM/10))),
    #          X_train_clean=X_train_clean, X_train_adv=X_train_adv, train_label=train_label,
    #          X_test_clean=X_test_clean, X_test_adv=X_test_adv, test_label=test_label)
    print("end")
