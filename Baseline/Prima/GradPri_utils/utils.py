...
from keras.models import clone_model, Model, Input
...
def NeuActInverse_confidence(img_input, hidden_layer_Model, partial_model_Model, mutant_rate):
    hidden_layer_output = hidden_layer_Model.predict(img_input)  # 获取模型的某一层输出
...
def GaussFuzz_confidence(img_input, model, model_tmp, mean, var, layer_name):
...
def WeightShuffl_confidence(img_input, model, model_tmp, layer_name, mutant_rate):
    # model_tmp = clone_model(model)
    weight_tmp = model_tmp.get_layer(layer_name).get_weights()
...
def Split_model(model, layer_num):
    weights = model.get_weights()   # 模型权重
    clon_model = clone_model(model)
    clon_model.set_weights(weights) # 给克隆模型权重

    # 模型的前一部分
    hidden_model = Model(inputs=clon_model.inputs, outputs=clon_model.layers[layer_num].output)

    # 模型的后一部分
    partial_model_input = Input(clon_model.layers[layer_num].output_shape[1:])
    partial_model_output = partial_model_input
    for layer in clon_model.layers[layer_num+1:]:
        partial_model_output = layer(partial_model_output)
    partial_model = Model(inputs=partial_model_input, outputs=partial_model_output)
    return hidden_model, partial_model
