...
def get_ats(
    ...
    if is_classification:
        ...
        pred = model.predict(dataset, batch_size=128, verbose=1)     # 预测样本置信度
        pred = np.argmax(pred, axis=1)
        if len(layer_names) == 1:
            layer_outputs = [
                temp_model.predict(dataset, batch_size=128, verbose=1)
            ]
        else:
            layer_outputs = temp_model.predict(
                dataset, batch_size=128, verbose=1
            )
...
def _get_train_target_ats(model, x_train, x_target, target_name, layer_names, args):
    ...
    saved_train_path = _get_saved_path(args.save_path, args.d, "train", layer_names)    # 需要保存文件的路径
    print("saved_train_path", saved_train_path)
    # if os.path.exists(saved_train_path[0]):     # ats文件已存在，不重复运行
...
def fetch_dsa(model, x_train, y_train, x_target, target_name, layer_names, args):
    """Distance-based SA
